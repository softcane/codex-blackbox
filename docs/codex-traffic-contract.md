...
does not emit legacy cache TTL/rebuild events, and does not make real Codex
traffic supported.
...
- `CodexTurnSummary` with Codex-native status, requested/served model, cached
  ...
  does not imply legacy cache TTL/rebuild behavior.
...
Phase 4C persists fixture Codex turns to SQLite without reusing legacy cache
...
For Codex rows, copied legacy cache columns remain non-authoritative:
...
from falling back to copied legacy pricing for Codex models.
...
- `test/e2e-openai-responses.sh` sends a fixture request through Envoy, verifies
  ...
  watch events, and asserts no legacy `CacheEvent` TTL/rebuild fields are
...
- No Codex cached input was represented as an legacy `CacheEvent`.
...
Local Codex stdout and hook-style enrichment were later explored, but that path
is now quarantined from the product-facing Codex surface. Normal
`coditor run -- codex exec ...` runs do not pass `--json`, and local stdout,
hooks, terminal output, or session files are not live telemetry sources.

## Phase 8A Codex Diagnostics

Phase 8A teaches diagnosis to read Codex-native signals from fixture Responses
turns and persisted Codex turn rows. It does not add real rate-limit parsing
and does not broaden the dogfood harness.
...
- Normal wrapper runs do not request Codex JSON stdout; Envoy-observed
  Responses traffic is the live telemetry source.
...
- Source of cwd/working directory in subscription traffic is request metadata
  when present, otherwise the AGENTS preamble path.
- Session id precedence across `session_id` header, response id,
