...
- `sessions.display_name`: persisted repo/session label. For live Codex
  subscription traffic this is inferred from request metadata when present, or
  from the AGENTS preamble path when metadata omits cwd.
- `sessions.initial_prompt`: first real user-visible prompt excerpt when
  available. Coditor strips the Codex AGENTS/environment preamble before storing
  this value so recall and validation markers point at the actual task.
...
- Real Codex prompt excerpts include injected AGENTS/environment preambles on
  the wire; Coditor strips those before persistence and stores repo display name
  separately.
...
Later live validation added CLI JSONL-derived telemetry for real Codex
`command_execution`, `mcp_tool_call`, and conservative skill events. These
events are forwarded through Coditor's hook endpoint and are visible in watch,
SQLite, Prometheus, and Grafana.

## Phase 7 Fake Codex Hook Contract

Phase 7 added the Codex hook/JSON telemetry endpoint:
...
`coditor.codex_hook.v1` fixture contract under `test/fixtures/`, plus equivalent
payloads generated by the `coditor run -- codex exec --json ...` wrapper from
Codex JSONL stdout. It always returns a safe JSON response with HTTP
`202 Accepted`; unknown events, missing fields, or invalid JSON are reported as
`ignored` rather than panicking. This endpoint is not in the model traffic path,
and hook/JSON telemetry failures must not affect Responses forwarding.
...
- `skill_expected`, `skill_fired`, `skill_failed`, `skill_missed`, and
  `skill_misfired` emit `SkillEvent` with bounded skill/source labels.
...
- Real Codex CLI JSONL currently provides local `command_execution`,
  `mcp_tool_call`, and skill-file-read signals. `command_execution` produces
  shell `ToolUse`/`ToolResult`, MCP JSONL produces `McpEvent`, and reading a
  `SKILL.md` file or a conservative skill announcement produces `SkillEvent`.
...
- Phase 8B dashboard panels for Codex Responses request count, token kinds,
...
- Live prompt excerpts now strip Codex's AGENTS/environment preamble, while
  repo display names are persisted separately.
- Real `codex exec --json` stdout is used as an enrichment source for shell
  tools, MCP calls, and conservative skill activity.
...
- Source of cwd/working directory in subscription traffic is request metadata
  when present, otherwise the AGENTS preamble path or hook/JSONL cwd context for
  enrichment details.
- Session id precedence across `session_id` header, hook id, response id,
  conversation id, and fallback hash.
- Whether future Codex versions keep emitting enough JSONL for complete
  tool/MCP/skill enrichment, or move those events into Responses SSE/hooks.
