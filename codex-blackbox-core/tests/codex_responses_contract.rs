...
fn request_parser_should_extract_codex_request_metadata_from_fixture() {
    ...
    assert_eq!(
        ...
        Some("codex-blackbox-fixture:/Users/pradeepsingh/code/codex-blackbox")
    );
    assert!(parsed.metadata.is_some());
    assert!(parsed.client_metadata.is_some());
    assert_eq!(
        parsed.cwd.as_deref(),
        Some("/Users/pradeepsingh/code/codex-blackbox")
    );
...
fn request_parser_should_extract_tool_request_shape_from_fixture() {
    ...
    assert_eq!(
        parsed.prompt_cache_key.as_deref(),
        Some("codex-blackbox-fixture:/Users/pradeepsingh/code/codex-blackbox:tools")
    );
    assert_eq!(
        parsed.cwd.as_deref(),
        Some("/Users/pradeepsingh/code/codex-blackbox")
...
fn fallback_session_identity_is_deterministic_and_splits_distinct_inputs() {
    let first = br#"{
      ...
      "input": "first task",
      "metadata": {
        "cwd": "/tmp/codex-blackbox-phase-2a"
      }
    }"#;
    let first_again = br#"{
      "model": "gpt-codex-fixture",
...
fn sse_accumulator_should_capture_tool_call_summary_from_fixture() {
    ...
    assert_eq!(
        summary.output_text,
        "The workspace defines codex-blackbox-core and codex-blackbox-cli."
    );
...
fn sse_accumulator_should_capture_failed_stream_fixture() {
    ...
    assert_eq!(
        ...
        Some("Fixture failure for Codex Blackbox contract tests.")
    );
...
#[test]
fn sse_accumulator_preserves_failed_terminal_usage_and_output() {
    let summary = accumulate_fixture(
        r#"data: {"type":"response.failed","response":{"id":"resp_failed_partial_001","model":"gpt-codex-fixture","output":[{"type":"message","content":[{"type":"output_text","text":"Partial output before failure."}]}],"usage":{"input_tokens":100,"input_tokens_details":{"cached_tokens":40},"output_tokens":20,"output_tokens_details":{"reasoning_tokens":5},"total_tokens":120},"error":{"message":"upstream failure after partial output"}}}

"#,
    );

    assert_eq!(summary.status, CodexResponseStatus::Failed);
    assert_eq!(summary.output_text, "Partial output before failure.");
    assert_eq!(
        summary.error_message.as_deref(),
        Some("upstream failure after partial output")
    );
    assert_eq!(summary.usage.input_tokens, 100);
    assert_eq!(summary.usage.cached_input_tokens, 40);
    assert_eq!(summary.usage.uncached_input_tokens, 60);
    assert_eq!(summary.usage.output_tokens, 20);
    assert_eq!(summary.usage.reasoning_output_tokens, 5);
...
fn codex_turn_accounting_maps_failed_and_incomplete_statuses() {
    ...
    assert_eq!(
        incomplete.incomplete_detail.as_deref(),
        Some("max_output_tokens")
    );
...
fn parse_minimal_request() -> codex_blackbox_core::codex_request::ParsedCodexRequest {
