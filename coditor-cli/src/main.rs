...
const CODITOR_CORE_URL: &str = "http://127.0.0.1:9091";
...
const CODEX_MODEL_PROVIDER_ID: &str = "coditor-chatgpt";
...
const BUNDLED_GRAFANA_DASHBOARD_JSON: &str = include_str!("../../grafana/dashboards/coditor.json");
...
fn envoy_proxy_url() -> String {
    std::env::var("CODITOR_ENVOY_PROXY_URL").unwrap_or_else(|_| ENVOY_PROXY_URL.to_string())
}
...
fn is_coditor_repo_root(path: &Path) -> bool {
...
fn coditor_data_dir() -> Result<PathBuf, String> {
    if let Some(dir) = std::env::var_os("CODITOR_HOME") {
        return Ok(PathBuf::from(dir));
    }
    if let Some(dir) = std::env::var_os("XDG_DATA_HOME") {
        return Ok(PathBuf::from(dir).join("coditor"));
    }
    if let Some(home) = std::env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".local/share/coditor"));
    }
    Err("Could not determine a data directory; set CODITOR_HOME.".to_string())
}
...
fn bundled_compose_yaml(stack_dir: &Path) -> String {
  ...
  coditor-core:
    image: ${{CODITOR_CORE_IMAGE:-{DEFAULT_CORE_IMAGE}}}
    expose:
      - "50051"
    ports:
      - "127.0.0.1:9091:9090"
    environment:
      - RUST_LOG=info
      - CODITOR_SESSION_BUDGET_DOLLARS=0
      - CODITOR_SESSION_BUDGET_TOKENS=0
      - CODITOR_CIRCUIT_BREAKER_THRESHOLD=5
    volumes:
      - coditor_data:/data
    healthcheck:
...
fn prepare_bundled_stack() -> Result<PathBuf, String> {
    let stack_dir = coditor_data_dir()?
        .join("stack")
    ...
    write_if_changed(
        &stack_dir.join("grafana/provisioning/dashboards/coditor.yml"),
    ...
    write_if_changed(
        &stack_dir.join("grafana/dashboards/coditor.json"),
...
fn resolve_compose_file() -> Result<PathBuf, String> {
    if let Some(path) = std::env::var_os("CODITOR_COMPOSE_FILE") {
        ...
        return Err(format!(
            "CODITOR_COMPOSE_FILE points to {}, but that file does not exist.",
            path.display()
        ));
    }

    let force_bundled = std::env::var_os("CODITOR_USE_BUNDLED_STACK").is_some();
    if !force_bundled {
...
fn coditor_container_for_port(port: u16) -> Option<String> {
    ...
    for line in stdout.lines() {
        ...
        if name.to_ascii_lowercase().contains("coditor") && ports.contains(&marker) {
...
fn port_state(port: u16) -> PortState {
    if is_port_available(port) {
        PortState::Available
    } else if let Some(container) = coditor_container_for_port(port) {
        PortState::CoditorService(container)
    } else {
        PortState::Busy
    }
}
...
async fn run_doctor() -> i32 {
    ...
    if docker_found && docker_daemon_running() {
        ...
        push_unique(
            &mut fixes,
            "Start Docker Desktop or your Docker daemon, then rerun `coditor up`.",
        );
    }
    ...
    for port in [10000, 9091, 3000] {
        match port_state(port) {
            ...
            PortState::CoditorService(container) => {
                print_check("✓", format!("port {port} used by Coditor ({container})"));
            }
            PortState::Busy => {
                failed = true;
                ...
                push_unique(
                    &mut fixes,
                    format!(
                        "Free port {port}, or stop the process using it before running `coditor up`."
                    ),
                );
            }
        }
...
async fn start_stack(no_grafana: bool) -> Result<(), String> {
    ...
    let compose = docker_compose_command()
        .ok_or_else(|| "docker compose is not available. Run `coditor doctor`.".to_string())?;
    ...
    println!("Starting Coditor stack with {}...", compose.display);
    ...
    let _ = io::stdout().flush();
    if !wait_for_health(CODITOR_CORE_HEALTH_URL, Duration::from_secs(90)).await {
        return Err(format!(
            "coditor-core did not become healthy at {CODITOR_CORE_HEALTH_URL}"
        ));
    }
...
fn current_cli_path() -> String {
    std::env::current_exe()
        ...
        .unwrap_or_else(|_| "coditor".to_string())
}

fn tmux_session_name() -> String {
    format!("coditor-watch-{}", std::process::id())
}

fn start_watcher() -> Result<WatchHandle, String> {
    let cli_path = current_cli_path();
    let core_url = coditor_core_url();
    if command_exists("tmux") {
...
fn print_child_run_status(plan: &ChildRunPlan) {
    match plan.mode {
        RunMode::CodexSubscriptionProxy => {
            ...
            eprintln!(
                "Coditor: model provider override {CODEX_MODEL_PROVIDER_ID} uses ChatGPT auth with Responses HTTP."
            );
            ...
            if !plan.env_removals.is_empty() {
                eprintln!(
                    "Coditor: removing inherited Codex parent-session env vars from the child process."
                );
            }
            if plan.requires_codex_observation {
                eprintln!("Coditor: will fail this run if coditor-core observes no Codex Responses request.");
            }
            if plan.stdin_mode == ChildStdinMode::Null {
                eprintln!("Coditor: child stdin is closed for Codex exec.");
            }
        }
        RunMode::PlainCommand => {
...
fn parse_codex_requests_total(metrics: &str) -> Option<f64> {
    ...
    for line in metrics.lines().map(str::trim) {
        if line.starts_with("coditor_requests_total{")
...
async fn fetch_codex_requests_total() -> Result<f64, String> {
    ...
    let metrics_url = format!("{}/metrics", coditor_core_url().trim_end_matches('/'));
    let resp = client
        .get(&metrics_url)
        .send()
        .await
    ...
    parse_codex_requests_total(&body).ok_or_else(|| {
        "coditor_requests_total for provider=\"codex_responses\" is missing".to_string()
    })
}
...
async fn run_child_command(watch_flag: bool, dry_run: bool, command: Vec<String>) -> i32 {
    ...
    let observed_before = if plan.requires_codex_observation {
        match fetch_codex_requests_total().await {
            Ok(value) => Some(value),
            Err(err) => {
                eprintln!("Error: Coditor observation pre-check failed: {err}");
                return 1;
            }
        }
    } else {
            ...
            if code == 0 {
                if let Some(before) = observed_before {
                    match wait_for_codex_observation_delta(before, Duration::from_secs(5)).await {
                        Ok(true) => {}
                        Ok(false) => {
                            eprintln!("Error: Codex exited successfully, but coditor-core did not observe any new provider=\"codex_responses\" request. Treating this as a failed Coditor proxy run.");
                            return 1;
                        }
                        Err(err) => {
                            eprintln!("Error: Coditor observation post-check failed: {err}");
                            return 1;
                        }
                    }
                }
...
Coditor removes inherited parent-session variables from child Codex runs:
  CODEX_CI
  CODEX_INTERNAL_ORIGINATOR_OVERRIDE
  CODEX_SHELL
  CODEX_THREAD_ID
...
mod tests {
    ...
    fn shell_and_yaml_quoting_preserve_command_arguments() {
        ...
        assert_eq!(
            shell_join(&["coditor".to_string(), "hello world".to_string()]),
            "coditor 'hello world'"
        );
    ...
    fn codex_run_plan_uses_subscription_proxy_config_overrides_by_default() {
        ...
        assert!(has_config_override(
            &plan.args,
            "model_provider=\"coditor-chatgpt\""
        ));
        assert!(has_config_override(
            &plan.args,
            "model_providers.coditor-chatgpt.base_url=\"http://127.0.0.1:10000/backend-api/codex\""
        ));
        assert!(has_config_override(
            &plan.args,
            "model_providers.coditor-chatgpt.wire_api=\"responses\""
        ));
        assert!(has_config_override(
            &plan.args,
            "model_providers.coditor-chatgpt.requires_openai_auth=true"
        ));
        assert!(has_config_override(
            &plan.args,
            "model_providers.coditor-chatgpt.supports_websockets=false"
        ));
    ...
    fn run_plan_preview_is_read_only_for_codex() {
        ...
        assert!(preview
            .contains("Model provider override: coditor-chatgpt (ChatGPT auth, Responses HTTP)"));
    ...
    fn codex_request_metric_parser_sums_codex_provider() {
        let metrics = r#"
# HELP coditor_requests_total Total requests
...
"#;
    ...
    fn parser_applies_command_defaults() {
        let cli = Cli::try_parse_from(["coditor", "watch"]).expect("watch parses");
        let Commands::Watch {
            url,
            no_cache,
            no_signals,
        ...
        let cli = Cli::try_parse_from(["coditor", "recall", "auth"]).expect("recall parses");
        let Commands::Recall {
            url,
            limit,
            days,
    ...
    fn codex_config_preview_is_read_only_and_subscription_only() {
        ...
        assert!(preview.contains(r#"-c 'model_provider="coditor-chatgpt"'"#));
        assert!(preview.contains(
